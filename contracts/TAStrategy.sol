...
contract TAStrategy is Initializable {
    ...
    function withdraw(uint amount) external onlyVault returns (uint WETHAmt) {
        ...
        if (mode) WETHAmt = withdrawWBTCETH(sharePerc);
        else WETHAmt = withdrawUSDCETH(sharePerc);
    ...
    function reimburse(uint amount) external onlyVault returns (uint WETHAmt) {
        if (mode) withdrawWBTCETH(amount * 1e18 / getWBTCETHPoolInETH());
        else withdrawUSDCETH(amount * 1e18 / getUSDCETHPoolInETH());
    ...
    function emergencyWithdraw() external onlyVault {
        // 1e18 == 100% of share
        if (mode) withdrawWBTCETH(1e18);
        else withdrawUSDCETH(1e18);
