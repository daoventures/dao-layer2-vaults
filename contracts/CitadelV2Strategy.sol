...
contract CitadelV2Strategy is Initializable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        WBTCETH.safeApprove(address(HBTCWBTCVault), type(uint).max);
        WBTCETH.safeApprove(address(sushiRouter), type(uint).max);
        DPIETH.safeApprove(address(HBTCWBTCVault), type(uint).max);
        DPIETH.safeApprove(address(sushiRouter), type(uint).max);
        DAIETH.safeApprove(address(HBTCWBTCVault), type(uint).max);
