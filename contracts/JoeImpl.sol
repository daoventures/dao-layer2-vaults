...
interface IMasterChef {
    ...
    function pendingTokens(uint pid, address user) external view returns (uint);
}
...
contract JoeImpl is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        token0.safeApprove(address(joeRouter), type(uint).max);
        token1.safeApprove(address(joeRouter), type(uint).max);
        lpToken.safeApprove(address(joeRouter), type(uint).max);
    ...
    function yield() external onlyOwnerOrAdmin whenNotPaused {
        masterChef.withdraw(poolId, 0); // To collect JOE
        uint joeBalance = JOE.balanceOf(address(this));
        uint WAVAXAmt = (joeRouter.swapExactTokensForTokens(
            joeBalance, 0, getPath(address(JOE), address(WAVAX)), address(this), block.timestamp
        ))[1];
        ...
        address baseToken = address(token0) == address(WAVAX) ? address(token1) : address(token0);
        uint baseTokenAmt = (joeRouter.swapExactTokensForTokens(
            WAVAXAmtHalf, 0, getPath(address(WAVAX), baseToken), address(this), block.timestamp
        ))[1];
        joeRouter.addLiquidity(address(baseToken), address(WAVAX), baseTokenAmt, WAVAXAmtHalf, 0, 0, address(this), block.timestamp);
    ...
    function getLpTokenPriceInAVAX() private view returns (uint) {
        ...
        uint baseTokenPriceInAVAX = (joeRouter.getAmountsOut(10 ** _baseTokenDecimals, getPath(baseToken, address(WAVAX))))[1];
    ...
    /// @return Pending rewards in JOE token
    ...
    function getPendingRewards() external view returns (uint) {
        uint pendingRewards = masterChef.pendingTokens(poolId, address(this));
        return pendingRewards + JOE.balanceOf(address(this));
    }
