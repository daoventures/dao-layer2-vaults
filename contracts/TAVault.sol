...
contract TAVault is Initializable, ERC20Upgradeable, OwnableUpgradeable, 
    ...
    function getAllPoolInETH() external view returns (uint) {
        ...
        if (USDCAmt > 1e6) {
            WETHAmt += sushiRouter.getAmountsOut(USDCAmt, getPath(address(USDC), address(WETH)))[1];
        }
        uint DAIAmt = DAI.balanceOf(address(this));
        if (DAIAmt > 1e18) {
            WETHAmt += sushiRouter.getAmountsOut(DAIAmt, getPath(address(DAI), address(WETH)))[1];
        }
        uint feesInETH;
        if (fees > 1e18) {
            // Assume fees pay in USDT
            feesInETH = sushiRouter.getAmountsOut(fees / 1e12, getPath(address(USDT), address(WETH)))[1];
        }
