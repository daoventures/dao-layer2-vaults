...
const unlockedAddress = "0x71f9ABEad2366Ef96d1CE42191972F29477A13A7"
const unlockedAddress2 = "0x71f9ABEad2366Ef96d1CE42191972F29477A13A7"
...
const setup = async () => {
    ...
    // const vault = await ethers.getContract("DaoSafuVaultTestnet", deployer)
    // const strategy = await ethers.getContract("DaoSafuStrategyTestnet", deployer)

    // let { btcbbnb, btcbbusd, btcbeth, cakebnb } = await deployDeps()

    let { vault, strategy } = await deploy("", "", "", "")
    ...
    await USDT.connect(unlockedUser).transfer(user1.address, ethers.utils.parseUnits("2", "18"))
    ...
    await USDT.connect(unlockedUser).transfer(user2.address, ethers.utils.parseUnits("2", "18"))
    await USDT.connect(unlockedUser).transfer(user3.address, ethers.utils.parseUnits("2", "18"))
...
describe("Citadel - DAI", async () => {
    ...
    it("Should work", async () => {
        ...
        await vault.connect(user1).withdraw(await vault.balanceOf(user1.address), USDT.address, [0,0,0,0])
        await vault.connect(user2).withdraw(await vault.balanceOf(user2.address), USDT.address, [0,0,0,0])
        await vault.connect(user3).withdraw(await vault.balanceOf(user3.address), USDT.address, [0,0,0,0])
