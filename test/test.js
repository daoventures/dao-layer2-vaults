...
describe("Metaverse-Farmer", () => {
    ...
    //     // console.log(ethers.utils.formatEther(await AXSETHVault.getAllPoolInUSD())) // 6000
    //     // console.log(ethers.utils.formatEther(await SLPETHVault.getAllPoolInUSD())) // 4500
    //     // console.log(ethers.utils.formatEther(await ILVETHVault.getAllPoolInUSD())) // 6000
    ...
    it("should work for UniswapV3 L1 GHSTETH vault contract", async () => {
            ...
            await WETHContract.deposit({value: ethers.utils.parseEther("2")})
            const sushiRouterAddr = "0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F"
            const sushiRouter = new ethers.Contract(sushiRouterAddr, ["function swapExactTokensForTokens(uint, uint, address[], address, uint) external"], client)
            ...
            await WETHContract.approve(sushiRouterAddr, ethers.constants.MaxUint256)
            await GHSTContract.approve(sushiRouterAddr, ethers.constants.MaxUint256)
            await sushiRouter.swapExactTokensForTokens(ethers.utils.parseEther("1"), 0, [WETHAddr, GHSTAddr], client.address, Math.floor(Date.now() / 1000))
            await WETHContract.transfer(client2.address, (await GHSTContract.balanceOf(client.address)).div(2))
            ...
            // // Invest
            // tx = await GHSTETHVault.connect(admin).invest()
            // // receipt = await tx.wait()
            // // console.log(receipt.gasUsed.toString())
    
            // // Second deposit
            // await GHSTETHContract.connect(client2).approve(GHSTETHVault.address, ethers.constants.MaxUint256)
            // await GHSTETHVault.connect(client2).deposit(ethers.utils.parseEther("1"))
            // // console.log(ethers.utils.formatEther(await GHSTETHVault.balanceOf(client2.address)))
    
            // // Check fees (deposit fees)
            // // console.log(ethers.utils.formatEther(await GHSTETHContract.balanceOf(treasury.address)))
            // // console.log(ethers.utils.formatEther(await GHSTETHContract.balanceOf(community.address)))
            // // console.log(ethers.utils.formatEther(await GHSTETHContract.balanceOf(strategist.address)))
    
            // // Second invest
            // tx = await GHSTETHVault.connect(admin).invest()
            // // receipt = await tx.wait()
            // // console.log(receipt.gasUsed.toString())
            // // console.log(ethers.utils.formatEther(await GHSTETHVault.getAllPoolInUSD())) // TODO: not correct
            ...
            // const binanceAcc = await ethers.getSigner(binanceAddr)
            // const SUSHIContract = new ethers.Contract(SUSHIAddr, IERC20_ABI, binanceAcc)
            // await SUSHIContract.transfer(GHSTETHVault.address, ethers.utils.parseEther("1"))
            ...
            // // Getter function
            // await network.provider.send("evm_mine")
            // console.log(ethers.utils.formatEther(await GHSTETHVault.getPendingRewards()))
            // console.log(ethers.utils.formatEther(await GHSTETHVault.getAllPool()))
            // console.log(ethers.utils.formatEther(await GHSTETHVault.getAllPoolInETH()))
            ...
            // // receipt = await tx.wait()
            // // console.log(receipt.gasUsed.toString())
            // console.log(ethers.utils.formatEther(await GHSTETHContract.balanceOf(client2.address)))
        })
})