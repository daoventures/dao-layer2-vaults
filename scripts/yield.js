...
async function main() {
    ...
    tx = await HBTCWBTCVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Curve HBTC-WBTC vault contract")

    const WBTCETHVault = await ethers.getContractAt("Sushi", WBTCETHVaultAddr, deployer)
    tx = await WBTCETHVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Sushi WBTC-ETH vault contract")

    const DPIETHVault = await ethers.getContractAt("Sushi", DPIETHVaultAddr, deployer)
    tx = await DPIETHVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Sushi DPI-ETH vault contract")

    const DAIETHVault = await ethers.getContractAt("Sushi", DAIETHVaultAddr, deployer)
    tx = await DAIETHVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Sushi DAI-ETH vault contract")
}
