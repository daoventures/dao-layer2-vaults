...
async function main() {
    ...
    tx = await mMSFTUSTVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Mirror MSFT-UST vault contract")

    const mTWTRUSTVault = await ethers.getContractAt("Mirror", mTWTRUSTVaultAddr, deployer)
    tx = await mTWTRUSTVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Mirror TWTR-UST vault contract")

    const mTSLAUSTVault = await ethers.getContractAt("Mirror", mTSLAUSTVaultAddr, deployer)
    tx = await mTSLAUSTVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Mirror TSLA-UST vault contract")

    const mGOOGLUSTVault = await ethers.getContractAt("Mirror", mGOOGLUSTVaultAddr, deployer)
    tx = await mGOOGLUSTVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Mirror GOOGL-UST vault contract")

    const mAMZNUSTVault = await ethers.getContractAt("Mirror", mAMZNUSTVaultAddr, deployer)
    tx = await mAMZNUSTVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Mirror AMZN-UST vault contract")

    const mAAPLUSTVault = await ethers.getContractAt("Mirror", mAAPLUSTVaultAddr, deployer)
    tx = await mAAPLUSTVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Mirror AAPL-UST vault contract")

    const mNFLXUSTVault = await ethers.getContractAt("Mirror", mNFLXUSTVaultAddr, deployer)
    tx = await mNFLXUSTVault.yield()
    await tx.wait()
    console.log("Success call yield for DAO L1 Mirror NFLX-UST vault contract")
}
